...
async function fetchRealNetflixToken(netflixId) {
    return new Promise((resolve, reject) => {
        ...
        const url = `https://ios.prod.ftl.netflix.com/iosui/user/15.48?${queryParams.toString()}`;
        ...
        if (proxyHost && proxyPort) {
            ...
            console.log(`[Proxy] Routing Netflix request via residential proxy: ${proxyHost}:${proxyPort}`);
        } else {
            console.log(`[Proxy] No proxy environment variables. Connecting directly.`);
        }
            ...
            res.on('end', () => {
                if (res.statusCode >= 400) {
                    return reject({
                        ...
                        message: `Netflix API returned HTTP ${res.statusCode}`,
                        body: body
                    });
                }
                try {
                    ...
                    if (token) {
                        ...
                        reject({
                            status: 200,
                            message: 'No token found in Netflix response structure',
                            body: body
                        });
                    }
                } catch (e) {
...
async function generateNftoken(netflixId) {
    try {
        console.log(`[generateNftoken] Requesting real Netflix token for ID: ${netflixId.substring(0, 10)}...`);
        const realToken = await fetchRealNetflixToken(netflixId);
        console.log(`[generateNftoken] Successfully retrieved real token from Netflix!`);
        return realToken;
    } catch (err) {
        console.error(`[generateNftoken] Failed to fetch real token:`, err.message || err);

        if (err.status === 401 || err.status === 403 || (err.status === 200 && err.message.includes('No token found'))) {
            console.log(`[generateNftoken] Invalid/expired cookie detected. Auto marking RED...`);
            try {
                const cookies = cookiesDB.read();
                const cookieRow = cookies.find(c => c.cookie_data.includes(netflixId));
                if (cookieRow) {
                    cookieRow.status = 'red';
                    cookieRow.notes = (cookieRow.notes ? cookieRow.notes + '\n' : '') + `[System] Auto RED on generate: Netflix returned status ${err.status || 200} (${err.message}) at ${new Date().toLocaleString()}`;
                    cookiesDB.write(cookies);
                    console.log(`[generateNftoken] Cookie ID ${cookieRow.id} has been marked RED.`);
                }
            } catch (dbErr) {
                console.error(`[generateNftoken] Database error:`, dbErr.message);
            }
            throw new Error('Sesi Netflix cookie sudah kadaluarsa atau tidak valid.');
        }

        console.log(`[generateNftoken] Proxy/Network error. Falling back to local dummy token.`);
        const payload = {
            id:  netflixId,
            ts:  Date.now(),
